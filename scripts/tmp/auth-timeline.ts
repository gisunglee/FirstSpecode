  ...
  const rows = await prisma.$queryRawUnsafe<Array<Record<string, unknown>>>(`
    WITH ev AS (
      SELECT r.creat_dt AS at, 'RT_ISSUED' AS ev, left(r.sesn_id,8) AS sesn, left(r.token_id,8) AS tok, r.auto_login_yn AS al, m.email_addr AS email
        FROM tb_cm_refresh_token r JOIN tb_cm_member m ON m.mber_id=r.mber_id WHERE r.creat_dt >= now() - interval '50 minutes'
      UNION ALL
      SELECT r.revoked_dt, 'RT_REVOKED', left(r.sesn_id,8), left(r.token_id,8), r.auto_login_yn, m.email_addr
        FROM tb_cm_refresh_token r JOIN tb_cm_member m ON m.mber_id=r.mber_id WHERE r.revoked_dt >= now() - interval '50 minutes'
      UNION ALL
      SELECT s.creat_dt, 'SESSION_NEW', left(s.sesn_id,8), NULL, NULL, m.email_addr
        FROM tb_cm_member_session s JOIN tb_cm_member m ON m.mber_id=s.mber_id WHERE s.creat_dt >= now() - interval '50 minutes'
      UNION ALL
      SELECT s.invald_dt, 'SESSION_INVALIDATED', left(s.sesn_id,8), NULL, NULL, m.email_addr
        FROM tb_cm_member_session s JOIN tb_cm_member m ON m.mber_id=s.mber_id WHERE s.invald_dt >= now() - interval '50 minutes'
    )
