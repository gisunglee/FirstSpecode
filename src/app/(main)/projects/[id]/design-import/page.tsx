...
type ImportFunction = { systemId?: string; name: string };
...
| requirementId | 선택 | 내보내기 JSON에 포함된 UUID를 그대로 유지. 신규 단위업무는 없어도 됨 |

**완료 기준**: 단위업무명 확보 (requirementId는 있으면 연결, 없으면 생략)
...
| 화면 유형 (screenType) | ✅ 권장 | LIST / DETAIL / GRID / TAB / FULL_SCREEN |
| 표시코드 (displayCode) | 선택 | Figma 등 디자인 도구 참조용 코드 (예: MBR_LIST) |
| 카테고리 (categoryL/M/S) | 선택 | 대/중/소 분류 |
| 설명 (description) | ✅ 권장 | 화면의 목적, 주요 기능, 사용자 흐름 |
...
| 항목 | 필수 | 허용 값 / 설명 |
|------|------|----------------|
| 영역명 (name) | ⭐ 필수 | 예: "검색 조건", "회원 목록", "버튼 영역" |
| 영역 유형 (areaType) | ✅ 권장 | SEARCH / GRID / FORM / DETAIL / BUTTON / TAB / CHART / OTHER |
| 설명 (description) | ✅ 권장 | 이 영역의 역할과 포함 내용 |
...
\`\`\`json
{
  "unitWorks": [
    {
      "requirementId": "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",  // 선택: 내보내기 JSON에 있으면 유지, 없으면 생략
      "name": "회원관리",
          ...
          "areas": [
            {
              ...
              "functions": [
                { "name": "이메일 검색", "description": "이메일 부분 일치 검색", "priority": "HIGH", "complexity": "LOW" },
                { "name": "상태 필터", "description": "활성/비활성/탈퇴 상태로 필터링", "priority": "MEDIUM", "complexity": "LOW" },
                { "name": "조회 버튼", "description": "입력한 조건으로 목록을 조회합니다", "priority": "HIGH", "complexity": "LOW" },
                { "name": "초기화 버튼", "description": "검색 조건을 초기 상태로 되돌립니다", "priority": "LOW", "complexity": "LOW" }
              ]
            },
            {
              "name": "회원 목록",
              ...
              "functions": [
                { "name": "회원 목록 표시", "description": "이메일, 이름, 가입일, 상태를 컬럼으로 표시", "priority": "HIGH", "complexity": "MEDIUM" },
                { "name": "페이징", "description": "페이지당 20건, 이전/다음 페이지 이동", "priority": "HIGH", "complexity": "LOW" },
                { "name": "상세 이동", "description": "행 클릭 시 회원 상세 화면으로 이동", "priority": "HIGH", "complexity": "LOW" }
              ]
            }
          ]
        }
...
| 필드 | 허용 값 |
|------|---------|
| screenType | \`LIST\` \| \`DETAIL\` \| \`GRID\` \| \`TAB\` \| \`FULL_SCREEN\` |
| areaType | \`SEARCH\` \| \`GRID\` \| \`FORM\` \| \`DETAIL\` \| \`BUTTON\` \| \`TAB\` \| \`CHART\` \| \`OTHER\` |
| priority | \`HIGH\` \| \`MEDIUM\` \| \`LOW\` |
| complexity | \`HIGH\` \| \`MEDIUM\` \| \`LOW\` |
| systemId | SPECODE 내보내기 JSON에 포함된 UUID 그대로 사용. 직접 작성 불가 |
| requirementId | SPECODE 내보내기 JSON에 포함된 경우 그대로 유지. 신규 단위업무는 없어도 됨 (선택) |
...
const JSON_TEMPLATE = `{
  "unitWorks": [
    {
      "requirementId": "UUID (신규 단위업무 등록 시 필수)",
      "name": "단위업무명 (예: 회원관리)",
          ...
          "areas": [
            {
              ...
              "functions": [
                { "name": "기능명", "description": "기능 설명", "priority": "MEDIUM", "complexity": "MEDIUM" }
              ]
            }
          ]
        }
