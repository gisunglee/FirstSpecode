...
function UnitWorksPageInner() {
  ...
  // URL ?assignedTo=me 로 진입한 경우 — 프로필 로드 후 전역 state에도 반영
  //   북마크/공유 URL로 들어온 사용자의 "me" 선호를 DB에도 영구 저장
  useEffect(() => {
  ...
  // ── 단위업무 목록 조회 ──────────────────────────────────────────────────────
  // 프로필(=전역 myAssigneeMode) 로드 전에는 쿼리 지연 → 첫 렌더 플리커 방지
  const { data, isLoading } = useQuery({
    queryKey: ["unit-works", projectId, filterReqId, effectiveAssignedTo],
    queryFn:  () => {
      // 여러 필터를 안전하게 조합하기 위해 URLSearchParams 사용
      const qs = new URLSearchParams();
      if (filterReqId) qs.set("reqId", filterReqId);
      if (effectiveAssignedTo) qs.set("assignedTo", effectiveAssignedTo);
