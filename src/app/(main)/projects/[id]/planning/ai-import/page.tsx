 ...
 *
 * 변경 이력:
 *   - 2026-04-25: P1 source 허용값을 단일 CRUD UI 와 일치(RFP/ADD/CHANGE).
 *                 과거 RFP/MEETING/INTERVIEW/ETC 였음 — Claude 출력이 UI 라벨과 안 맞던 문제.
...
| 상세 명세 (detailSpec) | ✅ 권장 | 요구사항 명세서 — **고객에게 제출되는 공식 문서**. 마크다운 표준 양식으로 작성 (아래 표준 양식 참조). 단일 UI "예시 삽입" 과 동일 양식 |
| 분석 노트 (discussionMd) | 선택 | 자유 형식 분석 기록 — 고객 인터뷰 내용, 인사이트, 협의 내용, VOC 등. **AI가 기획 시 참조하는 비공개 메모** |
| 우선순위 (priority) | ✅ 권장 | HIGH / MEDIUM / LOW |
| 출처 (source) | 선택 | RFP / ADD / CHANGE (기본값: RFP) — RFP=원문, ADD=추가 협의, CHANGE=변경 협의 |
| RFP 페이지 (rfpPage) | 선택 | 예: "p.23" 또는 "p.23-25" — RFP 원문에서 해당 요구사항이 기술된 위치 |
      ...
      "requirements": [
        {
          ...
          "detailSpec": "## 기능 개요\\n... (위 표준 양식 7섹션을 마크다운으로 채워 한 문자열에 \\\\n 으로 직렬화. 실제 출력에서는 줄바꿈 그대로) ...",
