  ...
  return (
    <div style={{ padding: 0, height: "100%" }}>
      ...
      <div style={{ margin: "16px 24px 0", border: "1px solid var(--color-border)", borderRadius: 8, overflow: "hidden", display: "flex", flexDirection: "column", height: "calc(100vh - 100px)" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "38% 62%", borderBottom: "1px solid var(--color-border)", flexShrink: 0 }}>
          {/* 좌측: 검색 */}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "38% 62%", flex: 1, overflow: "hidden" }}>
                  ...
                  {addingCode && (
                    <div style={{ padding: "8px 16px", borderBottom: "1px solid var(--color-border)", background: "var(--color-warning-subtle)" }}>
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 8 }}>
                        {/* 행 추가 — 점선 테두리는 "신규 입력행 추가" 시그니처 유지하되,
                          기존 회색 톤이 너무 묻혀서 브랜드 색으로 가시성 강화. */}
                        <button
                          onClick={addDraftRow}
                          style={{
          ...
          >
            ...
            <div style={{
              ...
              {deleteTarget.type === "group" ? (
                ...
                <>
                  <b>주의</b> — 이 코드를 다른곳에서 사용하고 있는지 확인 후 삭제하세요.
                </>
              )}
...
// 좁은 모니터에서 그룹 패널을 더 넓히기 위해 우측 코드 영역의 고정폭 컬럼을 타이트하게 조정.
//   사용/순서: 50px → 40px (토글·숫자만 표시되어 여백 축소 가능)
//   삭제:     32px → 24px
//   설명:     2fr  → 1.6fr (소폭 축소)
// 코드(fr)·코드명(fr) 비율은 그대로 — fr 기반이라 패널 폭에 자동 적응.
const CODE_GRID = "1.2fr 1.5fr 1.6fr 40px 40px 24px";
// 전체 조회 모드: 그룹코드명 컬럼 추가 (맨 앞)
const CODE_GRID_ALL = "1.5fr 1.2fr 1.5fr 1.6fr 40px 40px 24px";
