...
export default function WeeklyDocView({ projectId, monday }: { projectId: string; monday: string }) {
  ...
  // 곧바로 반영되어야 "같은 DB를 보고 있다"는 게 체감된다(정확한 키를 일일이 나열하기보다
  // prefix("work-log") 일치로 한 번에 무효화하는 쪽이 유지보수에 안전하다).
  const invalidateAll = () => queryClient.invalidateQueries({ queryKey: ["work-log"] });
      ...
      <table className="sp-doc-table">
        ...
        <thead>
          <tr>
            ...
            <th className="sp-doc-label">업무 내용</th>
            <th className="sp-doc-label">비고</th>
          </tr>
        </thead>
        <tbody>
          <tr>
          ...
          <tr>
            <td className="sp-doc-label">
              다음주 업무계획
              <div style={{ fontWeight: 400, fontSize: "var(--text-xs)", color: "var(--color-text-tertiary)" }}>
                {nextMonday} ~ {addDaysStr(nextMonday, 6)}
              </div>
            </td>
            <td colSpan={2}>
              <DocEditableCell
              ...
              <tr key={d}>
                <td>
                  <div style={{ fontWeight: 600, marginBottom: 4 }}>
                    {WEEKDAY_LABEL[i]}{" "}
                    <span style={{ fontWeight: 400, color: "var(--color-text-tertiary)", fontSize: "var(--text-xs)" }}>
                      ({d})
                    </span>
                  </div>
                  {items.length === 0 ? (
