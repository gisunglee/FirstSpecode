 ...
 *
 * 업무 리포트의 WeeklyDocView와 달리 "나만 보는 문서" 표 형식이 아니다 — 메인 콘텐츠는
 * 팀 전체를 모은 AI 초안이고, 그 아래 참여 현황(일별 기록 기준)과 팀원별 원본(접기/펼치기)을
 * 참고용으로 둔다.
 *
 * AI 원본(draft_cn)과 실제 보고서 내용(perf_cn/plan_cn/comment_cn/note_cn)을 분리했다
 * (2026-07-22) — "## 금주실적" 같은 헤더 문구로 구분하던 방식은 AI 출력에 오타가 나면 깨지기
 * 쉬웠고, AI가 쓴 것과 PM이 직접 쓴 것이 한 텍스트에 섞여 있어 구분도 안 됐다. 이제 draft_cn은
 * 참고용 읽기 전용("AI 원본" 배지)이고, PM은 그걸 보며 금주 실적/차주 계획/금주 코멘트/특이사항을
 * 별도 필드("직접 작성" 배지)에 직접 채운다 — AI 재생성을 눌러도 이 네 필드는 그대로 남는다
 * (덮어쓰지 않음). "총평"이라는 이름은 자기평가처럼 느껴진다는 피드백으로 "금주 코멘트"로
 * 바꿨고, 별도로 "특이사항"도 추가했다(comment_cn 컬럼명은 이전 review_cn에서 변경).
 *
...
export default function LeaderReportDetail({ projectId, monday }: { projectId: string; monday: string }) {
  ...
  const generateMutation = useMutation({
    mutationFn: () =>
      authFetch(`/api/projects/${projectId}/weekly-reports`, {
        method: "POST",
        body: JSON.stringify({ weekStartDt: monday }),
      }),
  ...
  const saveMutation = useMutation({
    mutationFn: (body: Partial<{ perfCn: string; planCn: string; commentCn: string; noteCn: string }>) =>
      authFetch(`/api/projects/${projectId}/weekly-reports/${existingReport!.weeklyReportId}`, {
        method: "PATCH",
        body: JSON.stringify(body),
      }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["weekly-report", projectId, existingReport?.weeklyReportId] }),
  });
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ border: "1px solid var(--color-border)", borderRadius: "var(--radius-card)", background: "var(--color-bg-card)", padding: 16 }}>
        ...
        {isGenerating && (
          <div style={{ fontSize: "var(--text-sm)", color: "var(--color-text-tertiary)", marginBottom: 8 }}>
        ...
        {existingReport && !isGenerating ? (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {/* AI 원본 — 참고용, 편집 불가. 여기서 내용을 보고 아래 항목에 직접 옮겨 적는다. */}
            <div style={{ border: "1px solid var(--color-border)", borderRadius: "var(--radius-md)", background: "var(--color-bg-muted)", padding: 14 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                ...
                <span style={{ fontSize: "var(--text-xs)", color: "var(--color-text-tertiary)" }}>참고용 — 아래 항목에 직접 옮겨 적어 주세요</span>
                <button type="button" className="sp-btn sp-btn-ghost sp-btn-xs" style={{ marginLeft: "auto" }} onClick={handleCopyAiDraft}>
                  {copied ? "복사됨" : "복사"}
                </button>
              </div>
              {reportDetailQuery.data?.draftCn ? (
                <AiFeedbackMarkdown value={reportDetailQuery.data.draftCn} />
              ) : (
                <div style={{ fontSize: "var(--text-sm)", color: "var(--color-text-tertiary)" }}>AI가 생성한 원본이 비어 있습니다.</div>
              )}
            </div>

            {/* 직접 작성 — perf_cn/plan_cn/comment_cn/note_cn 별도 컬럼. 헤더 문구 파싱에 의존하지
                않아 AI 원본의 오타나 형식 변화와 무관하게 항상 안정적으로 구분된다.
                네 항목을 한눈에 비교하기 좋게 가로로 배치(좁은 화면에서는 자동으로 줄바꿈). */}
            <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
              <div style={{ flex: "1 1 260px", minWidth: 0 }}>
                ...
                <DocEditableCell
                  ...
                  placeholder="AI 원본을 참고해 이번 주 실적을 정리해 보세요."
                  minRows={6}
              ...
              <div style={{ flex: "1 1 260px", minWidth: 0 }}>
                ...
                <DocEditableCell
                  ...
                  placeholder="AI 원본을 참고해 다음 주 계획을 정리해 보세요."
                  minRows={6}
          ...
          !isGenerating && (
            <div style={{ fontSize: "var(--text-sm)", color: "var(--color-text-tertiary)" }}>
              아직 이 주의 초안이 없습니다. "AI 요청"을 눌러 팀원들의 업무일지를 모아 초안을 만들어 보세요.
            </div>
          )
        )}
      </div>
            ...
            {rawMberIds.map((mberId) => {
              ...
              return (
                <div key={mberId} style={{ borderTop: "1px solid var(--color-border-subtle)", paddingTop: 10 }}>
                  ...
                  {weekLines.length > 0 && (
                    <ul style={{ margin: "0 0 6px", paddingLeft: 18, fontSize: "var(--text-xs)", color: "var(--color-text-secondary)" }}>
                      {weekLines.map((l, i) => <li key={i}>{l}</li>)}
                    </ul>
                  )}
                  {dailies.length > 0 && (
                    <ul style={{ margin: 0, paddingLeft: 18, fontSize: "var(--text-xs)", color: "var(--color-text-secondary)" }}>
