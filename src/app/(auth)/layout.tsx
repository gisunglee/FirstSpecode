 ...
 *     (가입·소셜 가입 완료·초대 수락에만 표시. 대상 경로는 _components/AuthShell.tsx 의 PANEL_PATHS)
 *   - 오른쪽: 폼 영역(light) — 로그인·회원가입·비밀번호 재설정·초대 수락 등 children 이 가운데 카드로 놓임
 *   - 패널이 없는 화면과 900px 이하(모바일)에서는 가운데 카드만 보인다
...
export default function AuthLayout({
  ...
  // 왼쪽 — 서비스 소개 패널. 어느 화면에 붙일지는 AuthShell 의 PANEL_PATHS 가 정한다.
  const panel = (
    <aside className="sp-auth-aside" data-theme="dark">
      <div className="sp-auth-aside-inner">
        <Link href="/intro" className="sp-auth-brand">
          <span className="sp-auth-brand-mark" aria-hidden="true"><BrandMark /></span>
          SPECODE
        </Link>
  ...
  return <AuthShell panel={panel}>{children}</AuthShell>;
}
