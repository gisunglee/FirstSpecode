/**
 * POST /api/projects/[id]/ai-tasks/[taskId]/rerun — AI 태스크 재실행
 *
 * 상태 제한 없이 기존 태스크를 복사하여 새 PENDING 태스크 생성
 * retry와 달리 DONE/APPLIED/PENDING 등 모든 상태에서 실행 가능
 */
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    // IN_PROGRESS 상태는 이미 처리 중이므로 재실행 차단
    if (task.task_sttus_code === "IN_PROGRESS") {
      return apiError("VALIDATION_ERROR", "처리 중인 태스크는 재실행할 수 없습니다.", 400);
    }

    // 기존 태스크 정보를 그대로 복사하여 새 PENDING 태스크 생성
    ...
    console.error(`[POST /api/projects/${projectId}/ai-tasks/${taskId}/rerun] DB 오류:`, err);
    return apiError("DB_ERROR", "재실행 처리 중 오류가 발생했습니다.", 500);
  }
}
