/**
 * GET /api/projects/[id]/unit-works/[unitWorkId]/export/docx
 *   — 단위업무 1건의 프로그램 사양서를 현재 시점 양식으로 Word(.docx) 다운로드
 *
 * 데이터 흐름:
 *   buildUnitWorkDocxWithHistory() 한 번에 (입력 매핑 → 발행이력 조합 → docx Buffer)
 *   내부 단계는 lib/exports/unit-work-data.ts 참조.
 *
 * 권한:
 ...
 *
 * 관련:
 *   - 발행 버전의 docx 다운로드는 /documents/release/[releaseId]/docx 별도 엔드포인트
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const result = await buildUnitWorkDocxWithHistory(projectId, unitWorkId);
    if (!result.ok) {
    ...
    console.error(`[GET /api/projects/${projectId}/unit-works/${unitWorkId}/export/docx] 오류:`, err);
    return apiError("EXPORT_ERROR", "프로그램 사양서 생성에 실패했습니다.", 500);
  }
}
