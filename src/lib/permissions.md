...
주요 잔존 영역: `ai-tasks` 본체 + 액션(retry/cancel/reject) / `areas·functions·requirements/files·ai·excalidraw·inline` / `reference-info` / `baseline` / `planning/bulk-import` / `design/bulk-import` / `phase-progress` / `col-mappings` / `impl-tree` / `*/sort` 11종 / `tasks/[taskId]/copy` / `screens/description-history` / `reviews/[reviewId]` 본체 등
