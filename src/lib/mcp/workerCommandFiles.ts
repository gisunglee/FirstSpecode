 ...
 *     원본 파일(.claude/commands/run-ai-tasks.md, task_complete.py)을 그대로 읽어 반환
 *
 * 사본을 만들지 않고 fs로 원본을 직접 읽는 이유:
 *   내용을 문자열 상수로 복사해두면 run-ai-tasks.md를 고칠 때마다 이 파일도
 *   같이 고쳐야 하고, 깜빡하면 고객에게 구버전이 나간다. 원본 하나만 두고
 *   요청 시점에 읽으면 항상 최신 상태가 보장된다.
 *
 * 배포 주의:
 *   Vercel 서버리스는 코드에서 import/require 하지 않는 파일은 빌드 트레이싱에서
 *   빠진다. 아래 두 파일은 next.config.ts의 outputFileTracingIncludes로 명시
...
export function getWorkerCommandFiles(): WorkerCommandFile[] {
  return WORKER_COMMAND_FILE_PATHS.map((relPath) => ({
    path:    relPath,
    content: fs.readFileSync(path.join(process.cwd(), relPath), "utf-8"),
  }));
}
