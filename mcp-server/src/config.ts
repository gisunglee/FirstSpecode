...
/** 서비스 계정 회원 ID */
export function getServiceMberId(): string {
  return process.env.SERVICE_MBER_ID || "";
}

/** 서비스 계정 이메일 */
export function getServiceEmail(): string {
...
export function validateConfig(): void {
  ...
  if (!getServiceMberId()) missing.push("SERVICE_MBER_ID");
  if (!getServiceEmail()) missing.push("SERVICE_EMAIL");

  if (missing.length > 0) {
    console.error(
      `[MCP Server] 필수 환경변수 누락: ${missing.join(", ")}\n` +
