...
export function registerTools(server) {
    ...
    // 1. 프로젝트 (Project)
    // ═══════════════════════════════════════════════════════════════
    server.tool("list_projects", "프로젝트 목록 조회 — 서비스 계정이 접근 가능한 프로젝트 목록을 반환합니다", {}, async () => {
        try {
    ...
    server.tool("get_project", "프로젝트 상세 조회 — 프로젝트 기본 정보와 내 역할을 반환합니다", { projectId: z.string().describe("프로젝트 ID") }, async ({ projectId }) => {
        try {
    ...
    //    계층: 과업 → 요구사항 → 사용자스토리
    // ═══════════════════════════════════════════════════════════════
    server.tool("list_tasks", "과업 목록 조회 — 프로젝트의 과업 목록과 요구사항 수/우선순위 집계를 반환합니다", {
    ...
    server.tool("get_task", "과업 상세 조회 — 과업명, 카테고리, 정의, 상세내용 등을 반환합니다", {
        projectId: z.string().describe("프로젝트 ID"),
    ...
    server.tool("create_task", "과업 생성 — 새 과업을 등록합니다. displayId(SFR-NNNNN)는 자동 채번됩니다", {
        projectId: z.string().describe("프로젝트 ID"),
        ...
        try {
            const data = await specodeFetch(`/api/projects/${projectId}/tasks`, { method: "POST", body: JSON.stringify(body) });
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("update_task", "과업 수정 — 기존 과업 정보를 업데이트합니다", {
        projectId: z.string().describe("프로젝트 ID"),
        taskId: z.string().describe("과업 ID"),
        name: z.string().describe("과업명 (필수)"),
        ...
        try {
            const data = await specodeFetch(`/api/projects/${projectId}/tasks/${taskId}`, { method: "PUT", body: JSON.stringify(body) });
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("delete_task", "과업 삭제 — 과업을 삭제합니다. deleteType으로 하위 데이터 처리 방식을 결정합니다", {
        projectId: z.string().describe("프로젝트 ID"),
        taskId: z.string().describe("과업 ID"),
        ...
        try {
            ...
            const data = await specodeFetch(`/api/projects/${projectId}/tasks/${taskId}${qs}`, { method: "DELETE" });
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    //    계층: 과업 → [요구사항] → 사용자스토리
    // ═══════════════════════════════════════════════════════════════
    server.tool("list_requirements", "요구사항 목록 조회 — 프로젝트의 요구사항 목록을 반환합니다 (검색, 페이지네이션 지원)", {
        projectId: z.string().describe("프로젝트 ID"),
        ...
        try {
            ...
            const data = await specodeFetch(`/api/projects/${projectId}/requirements${qs}`);
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("get_requirement", "요구사항 상세 조회 — 요구사항 정보와 연결된 사용자스토리를 반환합니다", {
        projectId: z.string().describe("프로젝트 ID"),
        requirementId: z.string().describe("요구사항 ID"),
    }, async ({ projectId, requirementId }) => {
        try {
            const data = await specodeFetch(`/api/projects/${projectId}/requirements/${requirementId}`);
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("create_requirement", "요구사항 생성 — 새 요구사항을 등록합니다. displayId(REQ-NNNNN)는 자동 채번됩니다. 과업에 소속시키려면 taskId를 전달하세요 (선행: list_tasks 또는 get_planning_tree로 taskId 조회)", {
        projectId: z.string().describe("프로젝트 ID"),
        ...
        try {
            const data = await specodeFetch(`/api/projects/${projectId}/requirements`, { method: "POST", body: JSON.stringify(body) });
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("update_requirement", "요구사항 수정 — 기존 요구사항 정보를 업데이트합니다", {
        projectId: z.string().describe("프로젝트 ID"),
        requirementId: z.string().describe("요구사항 ID"),
        name: z.string().describe("요구사항명 (필수)"),
        priority: z.string().describe("우선순위 (필수). 허용값: HIGH | MEDIUM | LOW"),
        ...
        try {
            const data = await specodeFetch(`/api/projects/${projectId}/requirements/${requirementId}`, { method: "PUT", body: JSON.stringify(body) });
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("delete_requirement", "요구사항 삭제 — 요구사항과 하위 사용자스토리를 삭제합니다", {
        projectId: z.string().describe("프로젝트 ID"),
        requirementId: z.string().describe("요구사항 ID"),
    }, async ({ projectId, requirementId }) => {
        try {
            const data = await specodeFetch(`/api/projects/${projectId}/requirements/${requirementId}`, { method: "DELETE" });
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    //    계층: 과업 → 요구사항 → [사용자스토리]
    // ═══════════════════════════════════════════════════════════════
    server.tool("list_user_stories", "사용자스토리 목록 조회 — 프로젝트의 사용자스토리 목록을 반환합니다 (요구사항별, 과업별 필터 가능)", {
        projectId: z.string().describe("프로젝트 ID"),
        ...
        try {
            ...
            const data = await specodeFetch(`/api/projects/${projectId}/user-stories${qs}`);
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("get_user_story", "사용자스토리 상세 조회 — 스토리 정보, 인수기준, 상위 요구사항/과업 정보를 반환합니다", {
        projectId: z.string().describe("프로젝트 ID"),
        storyId: z.string().describe("사용자스토리 ID"),
    }, async ({ projectId, storyId }) => {
        try {
            const data = await specodeFetch(`/api/projects/${projectId}/user-stories/${storyId}`);
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("create_user_story", "사용자스토리 생성 — 새 사용자스토리를 등록합니다. displayId(STR-NNNNN)는 자동 채번됩니다. 선행: list_requirements 또는 get_planning_tree로 requirementId를 조회하세요", {
        projectId: z.string().describe("프로젝트 ID"),
        ...
        try {
            const data = await specodeFetch(`/api/projects/${projectId}/user-stories`, { method: "POST", body: JSON.stringify(body) });
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("update_user_story", "사용자스토리 수정 — 기존 사용자스토리를 업데이트합니다", {
        projectId: z.string().describe("프로젝트 ID"),
        storyId: z.string().describe("사용자스토리 ID"),
        requirementId: z.string().describe("소속 요구사항 ID (필수)"),
        name: z.string().describe("스토리명 (필수)"),
        ...
        try {
            const data = await specodeFetch(`/api/projects/${projectId}/user-stories/${storyId}`, { method: "PUT", body: JSON.stringify(body) });
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("delete_user_story", "사용자스토리 삭제 — 사용자스토리와 하위 인수기준을 삭제합니다", {
        projectId: z.string().describe("프로젝트 ID"),
        storyId: z.string().describe("사용자스토리 ID"),
    }, async ({ projectId, storyId }) => {
        try {
            const data = await specodeFetch(`/api/projects/${projectId}/user-stories/${storyId}`, { method: "DELETE" });
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    // ═══════════════════════════════════════════════════════════════
    server.tool("get_planning_tree", "기획 트리 조회 — 과업 > 요구사항 > 사용자스토리 계층 구조를 반환합니다. 전체 기획 데이터를 한눈에 파악할 때 사용하세요", { projectId: z.string().describe("프로젝트 ID") }, async ({ projectId }) => {
        try {
            const data = await specodeFetch(`/api/projects/${projectId}/planning/tree`);
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    //    연결: 요구사항(reqId) ↔ 단위업무
    // ═══════════════════════════════════════════════════════════════
    server.tool("list_unit_works", "단위업무 목록 조회 — 프로젝트의 단위업무 목록을 반환합니다 (요구사항별 필터 가능). 진척률, 화면 수, AI 구현 요청 상태 포함", {
        projectId: z.string().describe("프로젝트 ID"),
        reqId: z.string().optional().describe("요구사항 ID (필터 — 특정 요구사항의 단위업무만)"),
    }, async ({ projectId, reqId }) => {
        try {
            const qs = buildQs({ reqId });
            const data = await specodeFetch(`/api/projects/${projectId}/unit-works${qs}`);
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("get_unit_work", "단위업무 상세 조회 — 단위업무 정보, 설명, 하위 화면 목록을 반환합니다", {
        projectId: z.string().describe("프로젝트 ID"),
        unitWorkId: z.string().describe("단위업무 ID"),
    }, async ({ projectId, unitWorkId }) => {
        try {
            const data = await specodeFetch(`/api/projects/${projectId}/unit-works/${unitWorkId}`);
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("create_unit_work", "단위업무 생성 — 새 단위업무를 등록합니다. displayId(UW-NNNNN)는 자동 채번됩니다. 선행: list_requirements로 reqId를 조회하세요 (상위 요구사항 필수)", {
        projectId: z.string().describe("프로젝트 ID"),
        ...
        try {
            const data = await specodeFetch(`/api/projects/${projectId}/unit-works`, { method: "POST", body: JSON.stringify(body) });
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("update_unit_work", "단위업무 수정 — 기존 단위업무 정보를 업데이트합니다", {
        projectId: z.string().describe("프로젝트 ID"),
        unitWorkId: z.string().describe("단위업무 ID"),
        name: z.string().describe("단위업무명 (필수)"),
        description: z.string().optional().describe("단위업무 설명 (마크다운 지원)"),
        ...
        try {
            const data = await specodeFetch(`/api/projects/${projectId}/unit-works/${unitWorkId}`, { method: "PUT", body: JSON.stringify(body) });
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("delete_unit_work", "단위업무 삭제 — 단위업무를 삭제합니다. 기본적으로 하위 화면도 함께 삭제됩니다", {
        projectId: z.string().describe("프로젝트 ID"),
        unitWorkId: z.string().describe("단위업무 ID"),
        ...
        try {
            ...
            const data = await specodeFetch(`/api/projects/${projectId}/unit-works/${unitWorkId}${qs}`, { method: "DELETE" });
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    //    계층: 단위업무 → [화면] → 영역 → 기능
    // ═══════════════════════════════════════════════════════════════
    server.tool("list_screens", "화면 목록 조회 — 프로젝트 내 화면 목록을 반환합니다 (단위업무별 필터 가능)", {
        projectId: z.string().describe("프로젝트 ID"),
        unitWorkId: z.string().optional().describe("단위업무 ID (필터)"),
    }, async ({ projectId, unitWorkId }) => {
        try {
            const qs = buildQs({ unitWorkId });
            const data = await specodeFetch(`/api/projects/${projectId}/screens${qs}`);
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("get_screen", "화면 상세 조회 — 화면 정보, 설명, 연결된 영역 목록을 반환합니다", {
        projectId: z.string().describe("프로젝트 ID"),
        screenId: z.string().describe("화면 ID"),
    }, async ({ projectId, screenId }) => {
        try {
            const data = await specodeFetch(`/api/projects/${projectId}/screens/${screenId}`);
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("create_screen", "화면 생성 — 새 화면을 등록합니다. 단위업무에 소속시키려면 unitWorkId를 전달하세요 (선행: list_unit_works로 조회)", {
        projectId: z.string().describe("프로젝트 ID"),
        ...
        try {
            const data = await specodeFetch(`/api/projects/${projectId}/screens`, { method: "POST", body: JSON.stringify(body) });
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("update_screen", "화면 수정 — 기존 화면 정보를 업데이트합니다", {
        projectId: z.string().describe("프로젝트 ID"),
        screenId: z.string().describe("화면 ID"),
        ...
        try {
            const data = await specodeFetch(`/api/projects/${projectId}/screens/${screenId}`, { method: "PUT", body: JSON.stringify(body) });
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("delete_screen", "화면 삭제 — 화면을 삭제합니다. 기본적으로 하위 영역도 함께 삭제됩니다", {
        projectId: z.string().describe("프로젝트 ID"),
        screenId: z.string().describe("화면 ID"),
        ...
        try {
            const qs = buildQs({ deleteChildren });
            const data = await specodeFetch(`/api/projects/${projectId}/screens/${screenId}${qs}`, { method: "DELETE" });
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    //    계층: 단위업무 → 화면 → [영역] → 기능
    // ═══════════════════════════════════════════════════════════════
    server.tool("list_areas", "영역 목록 조회 — 프로젝트 내 영역 목록을 반환합니다 (화면별 필터 가능)", {
        projectId: z.string().describe("프로젝트 ID"),
        screenId: z.string().optional().describe("화면 ID (필터)"),
    }, async ({ projectId, screenId }) => {
        try {
            const qs = buildQs({ screenId });
            const data = await specodeFetch(`/api/projects/${projectId}/areas${qs}`);
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("get_area", "영역 상세 조회 — 영역 정보, 설명, 연결된 기능 목록을 반환합니다", {
        projectId: z.string().describe("프로젝트 ID"),
        areaId: z.string().describe("영역 ID"),
    }, async ({ projectId, areaId }) => {
        try {
            const data = await specodeFetch(`/api/projects/${projectId}/areas/${areaId}`);
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("create_area", "영역 생성 — 새 영역을 등록합니다. 화면에 소속시키려면 screenId를 전달하세요 (선행: list_screens로 조회)", {
        projectId: z.string().describe("프로젝트 ID"),
        ...
        try {
            const data = await specodeFetch(`/api/projects/${projectId}/areas`, { method: "POST", body: JSON.stringify(body) });
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("update_area", "영역 수정 — 기존 영역 정보를 업데이트합니다", {
        projectId: z.string().describe("프로젝트 ID"),
        areaId: z.string().describe("영역 ID"),
        ...
        try {
            const data = await specodeFetch(`/api/projects/${projectId}/areas/${areaId}`, { method: "PUT", body: JSON.stringify(body) });
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("delete_area", "영역 삭제 — 영역을 삭제합니다. 기본적으로 하위 기능도 함께 삭제됩니다", {
        projectId: z.string().describe("프로젝트 ID"),
        areaId: z.string().describe("영역 ID"),
        ...
        try {
            const qs = buildQs({ deleteChildren });
            const data = await specodeFetch(`/api/projects/${projectId}/areas/${areaId}${qs}`, { method: "DELETE" });
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    //    계층: 단위업무 → 화면 → 영역 → [기능]
    // ═══════════════════════════════════════════════════════════════
    server.tool("list_functions", "기능 목록 조회 — 프로젝트 내 기능 목록을 반환합니다 (영역별 필터 가능)", {
        projectId: z.string().describe("프로젝트 ID"),
        areaId: z.string().optional().describe("영역 ID (필터)"),
    }, async ({ projectId, areaId }) => {
        try {
            const qs = buildQs({ areaId });
            const data = await specodeFetch(`/api/projects/${projectId}/functions${qs}`);
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("get_function", "기능 상세 조회 — 기능 정보, 입출력 항목, 비즈니스 규칙을 반환합니다", {
        projectId: z.string().describe("프로젝트 ID"),
        functionId: z.string().describe("기능 ID"),
    }, async ({ projectId, functionId }) => {
        try {
            const data = await specodeFetch(`/api/projects/${projectId}/functions/${functionId}`);
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("create_function", "기능 생성 — 새 기능을 등록합니다. 영역에 소속시키려면 areaId를 전달하세요 (선행: list_areas로 조회)", {
        projectId: z.string().describe("프로젝트 ID"),
        ...
        try {
            const data = await specodeFetch(`/api/projects/${projectId}/functions`, { method: "POST", body: JSON.stringify(body) });
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("update_function", "기능 수정 — 기존 기능 정보를 업데이트합니다", {
        projectId: z.string().describe("프로젝트 ID"),
        functionId: z.string().describe("기능 ID"),
        ...
        try {
            const data = await specodeFetch(`/api/projects/${projectId}/functions/${functionId}`, { method: "PUT", body: JSON.stringify(body) });
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("delete_function", "기능 삭제 — 기능을 삭제합니다", {
        projectId: z.string().describe("프로젝트 ID"),
        functionId: z.string().describe("기능 ID"),
    }, async ({ projectId, functionId }) => {
        try {
            const data = await specodeFetch(`/api/projects/${projectId}/functions/${functionId}`, { method: "DELETE" });
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("list_db_tables", "DB 테이블 목록 조회 — 프로젝트에 등록된 데이터베이스 테이블 목록을 반환합니다. " +
        ...
        try {
            const data = await specodeFetch(`/api/projects/${projectId}/db-tables`);
            return textResult(data);
        }
        catch (err) {
            return errorResult(err);
    ...
    server.tool("get_db_table", "DB 테이블 상세 조회 — 테이블 스키마와 컬럼 정보를 반환합니다", {
        projectId: z.string().describe("프로젝트 ID"),
