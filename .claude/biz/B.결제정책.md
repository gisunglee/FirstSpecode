...
- 해제 3경로: **재결제 성공 → 전부 즉시 해제** / 소유자 "활성화" → 상한 이하인 그 프로젝트만 해제(초과면 403 + 무엇을 줄여야 하는지 안내: 편집 멤버를 뷰어로 내리거나, 열려 있는 다른 프로젝트를 정리하거나, 구독) / 강등 직후 소유 프로젝트가 1개뿐이고 상한 이하면 **자동 해제**. 관리자는 소유자 대신 같은 판정으로 풀 수 있다(대행). **상한 초과 상태를 강제로 푸는 버튼은 없다** — 운영 판단으로 풀어야 하면 관리자가 회원에게 플랜을 부여(4단계)한 뒤 대행하면 판정을 통과한다.
