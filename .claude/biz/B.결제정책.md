...
- 상한 계산은 3곳만: 프로젝트 생성(+복사), 멤버 초대(+수락 시 재검사), 첨부 업로드. 일상 요청은 아무것도 세지 않는다. (2026-09-20 추가) 구독 좌석은 **뷰어 → 편집 역할 승격**에서도 검사한다 — 편집 멤버가 되는 모든 경로(초대·수락·승격)가 좌석을 먹어야 불변식이 지켜진다. 뷰어는 계속 무료.
