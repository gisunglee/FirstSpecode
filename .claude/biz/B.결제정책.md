...
> 최종 갱신: 2026-09-21 · 상태: **결제 기능 + 관리자 결제 운영 화면 운영 배포 완료 (Mock PG, 내부 사용자용)**. 로컬 = origin (`818152e`), 코드·DDL 대기 없음. 다음 = 운영 env·cron(사용자) → 사업자 정보 실제값 → 토스 가맹 심사 → 토스 어댑터
...
- 2026-09-23 사용자 제안·기획 합의: 구독 화면에 "좌석에 누가 포함되나요?" 펼치기. 사람 기준(중복 제거가 보이게)·프로젝트 칩은 멤버 관리 링크·역할 변경은 결제 화면에서 안 함·초대 중 편집 멤버 안내 포함·기본 접힘. 시작 카드·플랜 카드·축소 모달 세 곳.
