...
> 최종 갱신: 2026-09-23 · 상태: **결제 기능 + 관리자 결제 운영 화면 + 좌석 구성 펼치기 운영 배포 완료 (Mock PG, 내부 사용자용)**. 로컬 = origin, 코드·DDL 대기 없음. 개발 산출물 대기 없음 — 다음은 전부 사용자 몫(§7-1 env·cron·브라우저 점검 → §7-2 사업자 정보 → 토스 심사) 이고, 그 뒤 개발은 §7-3 토스 어댑터
...
- 해제 3경로: **재결제 성공 → 전부 즉시 해제** / 소유자 "활성화" → 상한 이하인 그 프로젝트만 해제(초과면 403 + 무엇을 줄여야 하는지 안내) / 강등 직후 소유 프로젝트가 1개뿐이고 상한 이하면 **자동 해제**. 관리자는 소유자 대신 같은 판정으로 풀 수 있다(대행). **상한 초과 상태를 강제로 푸는 버튼은 없다** — 운영 판단으로 풀어야 하면 관리자가 회원에게 플랜을 부여(4단계)한 뒤 대행하면 판정을 통과한다.
...
- **환불 원장** — 실행은 PG 콘솔에서 수동(v1), 실행 뒤 관리자 화면에서 기록한다. 유형은 두 가지만:
  - `WITHDRAWAL` 청약철회: 구독 시작 결제(INITIAL)·승인 후 7일 이내·유료 기능 미사용(3플래그)을 **서버가 검사**. 잔액 전액. 같은 트랜잭션에서 **구독 즉시 종료**(사유 REFUND_WITHDRAWAL, 강등·잠금·메일). 조건에 안 맞으면 거부(409) — 운영 보정으로 기록하라고 안내.
...
0. **브라우저 확인(사용자)** — 관리자 계정으로 시스템 관리 > 결제(`/admin/billing`): 요약 카드·구독 탭(빈 목록)·결제 이력 탭·엑셀 버튼, 감사 로그 드롭다운에 결제 액션 5개·대상 2개. 지금은 구독 0건이라 숫자 대부분 0, 마지막 배치 카드는 cron 이 없어 "실행 기록 없음"이 정상.
