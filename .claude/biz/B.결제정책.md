...
- **(과금 구멍) MCP 키 공유로 인한 좌석 우회** — 2026-09-26 확인. MCP 키는 발급한 회원 자격으로 인증되므로, 결제자가 키 하나를 여러 명(미가입자 포함)에게 주면 그 사람들이 좌석을 쓰지 않고 그 프로젝트를 편집한다. 좌석은 `tb_pj_project_member` 행으로 세는데 키 사용자는 멤버 행이 생기지 않기 때문. 권한 확대는 없다(프로젝트 1개 고정·결제 API 차단·시스템 관리자 무효·잠금 적용) — 과금과 감사 추적의 문제다. 부수 문제: 모든 변경이 키 소유자 한 명의 행위로 기록됨(누가 했는지 알 수 없음), 개별 회수 불가(폐기하면 전원 끊김), **키에 유효기간이 없음**(`revoke_dt` 수동 폐기만), MCP 키 경로에 rate limit 없음.
  - 대응 방침: 자격증명 공유는 근본 차단이 불가능하므로 **억제**만 한다. ① 키 발급 화면 경고 문구 ② 키 단위 rate limit(`checkRateLimit` 재사용) — 이 둘까지. ③ 유효기간(컬럼+DDL)·④ 동시 사용 이상 탐지는 내부 사용자 단계에선 운영 부담이 더 크다고 보아 보류.
  - 장기 방향(사용자 의견): Claude Code 처럼 **키 대신 로그인(OAuth 등)** 으로 MCP 를 인증하게 하면 사람별 신원이 생겨 좌석·감사 문제가 같이 풀린다. 별도 과제.
