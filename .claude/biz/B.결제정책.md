...
> 최종 갱신: 2026-09-26 · 상태: **과금 정책 개정(FREE = 편집자 소유자 1명·열린 프로젝트 1개·뷰어 무제한, 환불 = 첫 결제 7일 무조건·계정당 1회) 코드 완료 — 커밋·푸시는 사용자 확인 대기(§7-1 0번)**. 운영에 배포된 것은 그 직전 상태(결제 기능 + 관리자 결제 운영 화면 + 좌석 구성 펼치기, Mock PG, 내부 사용자용). DDL 없음. 다음은 §7-1(푸시·배포 확인 → env·cron·브라우저 점검) → §7-2 사업자 정보 → 토스 심사, 그 뒤 개발은 §7-3 토스 어댑터
...
- [ ] **이용약관·개인정보처리방침 시행일** — `src/lib/consent.ts` `CURRENT_CONSENT_VERSION`(단일 출처, 화면 표기는 siteInfo 가 변환), 현재 "2026-10-01" 가정. 실제 오픈일로. 동의 기록 버전값이므로 오픈 전에 확정.
