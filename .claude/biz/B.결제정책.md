...
- [x] 관리자 결제 화면 — 요약·구독 목록·결제 이력(엑셀)·구독 상세 운영 액션 5개·관리자 알림 메일 (2026-09-21 코드)
- [x] 기획·GPT 교차 검토 반영 — 환불 원장(2유형·원 결제 참조·부분 환불·행 잠금), 결제 작업 토큰, 종료 사유, 강제 해제 제거, 감사 기록 트랜잭션 보장·순수 상수 분리·필터 등록, 승인 순액 지표, admin.ts 분리·라벨 통합, 동시성 스모크 (2026-09-21 코드, **DDL·푸시 대기**)
...
- 2026-09-21 GPT 교차 검토 수용·조정(사용자): 환불 = 청약철회(전액·서버 검사·구독 종료)/운영 보정(전액·부분·유지) 2유형 + 원 결제 참조·PARTIALLY_REFUNDED·행 잠금 · 결제 작업 토큰(구독 컬럼 2개, 별도 테이블 없음) · 종료 사유 컬럼 지금 추가 · 강제 해제 제거(대안 = 플랜 부여 → 대행) · 금융 액션 감사 같은 트랜잭션 · 승인 순액 지표 · 5코드/intent 테이블/해지 요청 알림은 축소. DDL 1건 확인 대기.
