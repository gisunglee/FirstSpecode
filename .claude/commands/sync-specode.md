...
`$ARGUMENTS`에서 `UW-XXXXX` 패턴을 모두 찾는다.

- **0개**면 전체 프로젝트를 실행하지 말고 “단위업무별 실행이 범위와 정확도가 가장
  좋습니다. 동기화할 UW 번호를 알려주세요.”라고 묻고 종료한다. UW 번호 없이는 어떤
  경우에도 진행하지 않는다.
- **2개 이상**이면(중복 지정) 어느 것을 실행할지 임의로 고르지 말고 “UW 번호가 여러 개
  감지되었습니다: {목록}. 한 번에 하나의 UW만 동기화할 수 있습니다. 정확히 하나만
  알려주세요.”라고 묻고 종료한다.
- **정확히 1개**일 때만 그 값을 실행 대상 UW로 확정하고 다음 단계로 진행한다.
