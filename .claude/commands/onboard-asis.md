...
### 사업 범위 구분 — `scopeStatus`

`create_unit_work`/`create_screen`/`create_area`/`create_function` 에는
`scopeStatus` 입력이 있다. 이 명령으로 등록하는 항목은
**예외 없이 `"EXISTING"`(이전 사업 그대로)를 명시해서 넘긴다.**

생략하면 서버가 `NEW`(이번 사업에서 새로 만든 것)로 저장한다. 그러면 1차 시스템을
읽어서 넣은 AS-IS 항목이 2차 사업 산출물과 한 덩어리가 되고, 이후에는 소스를 다시
뒤지지 않는 한 둘을 갈라낼 방법이 없다. 공수·진척 집계에도 그대로 섞여 들어간다.

항목마다 신규/수정 여부를 스스로 판단하지 않는다. 이 명령의 등록분은 전부
`EXISTING` 이다. 예외는 단 하나 —

**이미 등록된 항목과 같은 것을 발견하면 새로 만들지 말고 보고한다.**
1차 시스템을 훑다 보면 2차 사업으로 이미 등록해 둔 화면·기능과 같은 대상이 나온다.
그때 새로 `create_*` 하면 같은 것이 두 벌 생겨 트리가 망가진다. 등록하지 말고
"기존 `[FID-00012] 이메일 중복확인` 과 같은 대상으로 보입니다" 처럼 사용자에게
알린다. 그 항목을 `MODIFIED`(이전 사업에 있었고 이번 사업에서 수정)로 내릴지는
사용자가 판단한다 — `update_*` 로 임의 변경하지 않는다.
